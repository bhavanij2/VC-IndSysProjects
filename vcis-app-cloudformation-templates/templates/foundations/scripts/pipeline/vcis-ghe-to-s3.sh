...
#./vcis-ghe-to-s3.sh ENV=dev PROJECT=vc GHE_REPOSITORY_NAME=vcis-sam-sample-app-two BRANCH_NAME=develop S3_SOURCE_STACK=vcis-app-non-prod-account-level-resources PROJECT_MODULE_SSM=/vcis/dev/infra/tags/project-module-multiplication
...
if [[ $# -eq 0 ]]; then
    ...
    printf "\033[0;33mWhich is the CFN stack name where S3 bucket should be gotten from? (enter for vcis-app-non-prod-account-level-resources):\033[0m\n" & \
    echo -n ""
    read app_account_level_resources_stack_name

    if [[ -z ${app_account_level_resources_stack_name} ]]; then
      app_account_level_resources_stack_name="vcis-app-non-prod-account-level-resources"
    fi
...
#printf "\033[0;33mFeature associated to the artifact (enter for no feature)\033[0m\n" & \
#echo -n ""
#read feature
...
stack_name="vcis-app-ghe-to-s3-"${ghe_repository_name}"-"${env}

#build_output_path=${feature}
build_output_path=''
...
aws cloudformation ${cfn_operation} \
    ...
    ParameterKey=AppAccountLevelResourcesStackName,ParameterValue=${app_account_level_resources_stack_name} \
